...
void readMessageFromBuffer(char *message, int maxLength) {
   ...
   while (bufferPosition < maxLength) {
      bSuccess =
          ReadFile(g_hChildStd_OUT_Rd, messageBuffer, bufSize, &dwRead, NULL);
...
static void initPipe(void) {
   ...
   if (child == 0) {
      ...
      execlp("java", "java", option, "-classpath", classpath,
...
static string boolString(bool flag) { return (flag) ? "true" : "false"; }
...
void clearOp(GWindow gw) { putPipe("GWindow.clear(\"0x%lX\")", (long)gw); }

void repaintOp(GWindow gw) { putPipe("GWindow.repaint(\"0x%lX\")", (long)gw); }
...
void playSoundOp(Sound sound) { putPipe("Sound.play(\"0x%lX\")", (long)sound); }
...
void setRaisedOp(G3DRect rect, bool raised) {
   putPipe("G3DRect.setRaised(\"0x%lX\", %s)", (long)rect, boolString(raised));
}
...
void addOp(GCompound compound, GObject gobj) {
   putPipe("GCompound.add(\"0x%lX\", \"0x%lX\")", (long)compound, (long)gobj);
}
