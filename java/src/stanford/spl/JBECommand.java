...
class GImage_getPixelArray extends JBECommand {
	public void execute(TokenScanner scanner, JavaBackEnd jbe) {
		...
		if (gobj != null) {
			try {
				System.out.println("result:GPixelArray(" + gobj.getWidth() + ", " + gobj.getHeight() + ")");
				System.out.flush();
			} catch (Exception ex) {
				System.out.println("result:" + ex.getMessage());
				System.out.flush();
...
class GChooser_addItem extends JBECommand {
	 ...
	 * reflection to call the addItem method to avoid generating warnings in
	 * Java 7 (which added a type parameter to the JComboBox class) while still
	 * allowing compilation in Java 6.
	 */
	public void execute(TokenScanner scanner, JavaBackEnd jbe) {
		scanner.verifyToken("(");
		String id = nextString(scanner);
